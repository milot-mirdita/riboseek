...
InfModel loadModel(const std::string &cmtext) {
    InfModel m;
    ...
    m.cm->search_opts |= CM_SEARCH_INSIDE;
...
InfHit scanRegionInfernal(const InfModel &model, CM_t *tcm, const std::string &regionSeq) {
    ...
    float sc = 0.0f;
    const int scanStatus = FastIInsideScan(tcm, errbuf, tcm->smx, SMX_QDB1_TIGHT, dsq,
                                           1, Lwin, 0.0f, th, TRUE /*do_null3*/, 0.0f,
                                           NULL, NULL, NULL, &sc);
    ...
    tcm->align_opts = CM_ALIGN_NONBANDED | CM_ALIGN_CYK;
    ...
    const int adStatus = cm_alidisplay_Create(tcm, errbuf, adata, winSq, hStart,
                                              -1.0 /*tau: no HMM bands*/, watch->elapsed, &ad);
...
int cmscan(int argc, const char **argv, const Command &command) {
    ...
    for (size_t qi = 0; qi < queryRefs.size(); ++qi) {
        ...
        InfModel model = loadModel(cmText);
        if (!model.valid) {
        ...
        const size_t rid = resultReader.getId(ref.key);
        if (rid != UINT_MAX) {
...
#pragma omp for schedule(dynamic, 1) nowait
                for (long ci = 0; ci < static_cast<long>(cands.size()); ++ci) {
                    ...
                    if (cand.hasRegionCoord) {
                        const int pad = (W > 0) ? W : std::max(1, model.clen);
...
#pragma omp critical
        ...
        resultWriter.writeEnd(ref.key, 0);
        freeModel(model);
    }
